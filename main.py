...
class CurrencyConverter:
    def __init__(self, root):
        ...
        if not self.api_key:
            messagebox.showerror("Ошибка", "API‑ключ не найден. Установите переменную окружения EXCHANGE_RATE_API_KEY.")
            self.root.destroy()
            return
    ...
    # =================== Работа с API и кэширование ===================
    def get_exchange_rate(self, from_curr, to_curr):
        """Возвращает курс from_curr -> to_curr через API с кэшированием."""
        now = datetime.now()
        # Обновляем кэш раз в час
        if self.cache_time is None or (now - self.cache_time).total_seconds() > 3600:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data["result"] == "success":
                        self.rates_cache = data["conversion_rates"]
                        self.cache_time = now
                    else:
        ...
        if to_curr in self.rates_cache:
            return self.rates_cache[to_curr]
        else:
